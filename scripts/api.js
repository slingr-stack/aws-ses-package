...
function handleRequestWithRetry(requestFn, options, callbackData, callbacks) {
    try {
        ...
        sys.logs.info("[awsses] Handling request...: "+ JSON.stringify(error));
    }
}
...
exports.utils = {
    ...
    getConfiguration: function (property) {
        if (!property) {
            sys.logs.debug('[awsses] Get configuration');
            return JSON.stringify(config.get());
        }
        sys.logs.debug('[awsses] Get property: '+property);
...
function setApiUri(options) {
    ...
    options.url = "https://email."+config.get("region")+".amazonaws.com" + url;
    sys.logs.debug('[awsses] Set url: ' + options.path + "->" + options.url);
    return options;
}

function setAuthorization(options) {
    sys.logs.debug('[awsses] Setting header token oauth');
